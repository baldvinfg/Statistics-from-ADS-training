...
bald_vs_seb <- ggplot(raw_DATA, aes(x = analyser, y = g_ratio, color = analyser)) +
  geom_point(shape = 16, size = 3, alpha = 0.5, position = position_jitter(width = .1)) +
  ...
  scale_color_manual(values = c("Baldvin" = "#00BFC4", "Sebastian" = "#F8766D")) +  # Adjusted color names
  labs(
...
# Add p-value annotation, assuming wilcox_test_result has been calculated
p_value_text <- ifelse(wilcox_result$p.value < 0.0001, "p < 0.0001", sprintf("p = %.4f", wilcox_result$p.value))
bald_vs_seb <- bald_vs_seb + annotate("text", x = 1.5, y = 0.925, label = p_value_text, size = 5, color = "black")
...
bald_vs_seb_axondiam <- ggplot(raw_DATA, aes(x = analyser, y = axon_diameter, color = analyser)) +
  ...
  geom_hline(yintercept = seq(0, 2500, by = 250), linetype = "dashed", color = "gray")  # Add dashed lines

# Add p-value annotation, assuming wilcox_test_result has been calculated
p_value_text <- ifelse(wilcox_result$p.value < 0.0001, "p < 0.0001", sprintf("p = %.4f", wilcox_result$p.value))
bald_vs_seb_axondiam <- bald_vs_seb_axondiam + annotate("text", x = 1.5, y = 2400, label = p_value_text, size = 5, color = "black")
...
ggplot(raw_DATA, aes(x = neuron_number, y = g_ratio, group = analyser, color = analyser)) +
  ...
  labs(x = "Neuron", y = "G-ratio", title = "Comparison of G-ratio by Neuron", color = "Analyser") +  # Set the legend title
  scale_color_manual(values = c("Baldvin" = "blue", "Sebastian" = "red"))

ggsave("Sebastian vs Baldvin/Gratio_results_Seb_vs_Bald_indiv_axon_w_line.png", bg = "transparent", dpi = 300)

# And now for axon diameter
ggplot(raw_DATA, aes(x = neuron_number, y = axon_diameter, group = analyser, color = analyser)) +
  geom_point(position = position_dodge(width = 0.2)) +
  geom_line(aes(group = neuron_number), position = position_dodge(width = 0.2)) +
  geom_hline(yintercept = seq(0.5, 0.9, by = 0.05), linetype = "dashed", color = "grey") +
  scale_y_continuous(breaks = seq(0, 2500, 250), limits = c(0, 2500), expand = c(0, 0)) +
  ...
  labs(x = "Neuron", y = "Axon diameter (nm)", title = "Comparison of Axon diameter by Neuron", color = "Analyser") +  # Set the legend title
  scale_color_manual(values = c("Baldvin" = "blue", "Sebastian" = "red"))
...
bland_altman_plot <- ggplot(bland_altman_data, aes(x = Average, y = CenteredDifference)) +
  ...
  annotate("text", x = max(bland_altman_data$Average), y = upper_limit, label = sprintf("Upper Limit: %.5f", upper_limit), hjust = 1, vjust = -1, size = 3) +
  annotate("text", x = max(bland_altman_data$Average), y = lower_limit, label = sprintf("Lower Limit: %.5f", lower_limit), hjust = 1, vjust = 1.5, size = 3) +
...
ggsave("Sebastian vs Baldvin/Bland_Altman_plot.png", bg="white", dpi = 300)
```
...
ggplot(combined_data, aes(x = Baldvin_g_ratio, y = Sebastian_g_ratio)) +
  ...
  scale_x_continuous(breaks = seq(0.35, 1, 0.1), limits = c(0.35, 1), expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(0.35, 1, 0.1), limits = c(0.35, 1), expand = c(0, 0)) +
...
ggsave("Sebastian vs Baldvin/Baldvin_vs_Sebastian_line_of_fit.png", bg = "white", dpi = 300)
...
ggsave("Tracings to check variability/gratio_variability_final_dot_box.png", width = 10, height = 8, dpi = 300, bg = "white")
```
...
morph_test_61073_selection = subset(morph_test_61073, axon_diam..um. >= 0.2, myelin_thickness..um. > 0.01)
...
```{r}
# Let's compare the test model and manual tracings
joined_data_man_test_61073_final$name <- factor(joined_data_man_test_61073_final$name, levels = c("test", "manual"))

test_model_manual_graph <- ggplot(joined_data_man_test_61073_final, aes(x = name, y = gratio, col = name)) +
  geom_point(shape = 16, size = 5, alpha = 0.6, position = position_jitter(width = .1)) +
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "test" = "model_test")) +
  ...
  geom_hline(yintercept = seq(0.45, 0.95, by = 0.05), linetype = "dashed", color = "gray")  # Add dashed lines

# Add p-value annotation
test_model_manual_graph <- test_model_manual_graph + annotate("text", x = 1.5, y = 0.925, label = sprintf("p = %.4f", t_test_result_61073$p.value), size = 5, color = "black")
...
ggplot(model_compare_man_test_61073, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
...
morph_test_all_final = subset(morph_test_all, axon_diam..um. >= 0.2, myelin_thickness..um. > 0.01)
...
#t_test_result_all <- t.test(joined_x_final_final$gratio.x, joined_x_final_final$gratio.y, paired = TRUE)
#t_test_result_all
...
formatted_p_value <- ifelse(wilcox_test_result$p.value < 0.0001, "p < 0.0001", sprintf("p = %.4f", wilcox_test_result$p.value))
...
ggplot(joined_x_final_final, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_all, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
...
ggplot(model_compare_gratio_difference_all, aes(x = gratio.x, y = diff_gratio)) +
  ...
  labs(title = "Difference in Gratio (ground truth vs. test_model)",
       x = "Gratio",
       y = "Difference in Gratio") +
  theme_minimal() +
  theme(
...
# Won't use here! Let's perform a paired t-test on my filtered data
t_test_result_all_new <- t.test(joined_z_final_final$gratio.x, joined_z_final_final$gratio.y, paired = TRUE)
...
test_all_new_manual_graph <- ggplot(joined_data_man_test_all_new_final, aes(x = name, y = gratio, col = name)) +
  ...
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "test" = "ADS test model")) +
  scale_y_continuous(breaks = seq(0.25, 1, 0.05), limits = c(0.25, 1), expand = c(0, 0)) +
  scale_color_manual(values = c("manual" = "#00BFC4", "test" = "#F8766D")) +
  labs(
    title = "G-ratio comparison of Ground truth vs test model",
    x = "Measurement Method",
  ...
  geom_hline(yintercept = seq(0.25, 1, by = 0.05), linetype = "dashed", color = "gray")  # Add dashed lines
...
## IMPORTANT: There are values here that are under 0.35, for the thesis (and maybe the presentation in Iceland, I will need to change the y-axis for the conventional fixative!!!)
...
ggplot(joined_z_final_final, aes(x = gratio.x, y = gratio.y)) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_all_new, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
  scale_x_continuous(breaks = seq(0.3, 1, 0.1), limits = c(0.3, 1), expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(0.3, 1, 0.1), limits = c(0.3, 1), expand = c(0, 0)) +
  labs(
    x = "G-ratio of the Ground Truth", # new x label
...
ggplot(model_compare_gratio_difference_all_new, aes(x = gratio.x, y = diff_gratio)) +
  ...
  annotate("text", x = max(bland_altman_data$Average), y = 0.011615, label = sprintf("Upper Limit: %.5f", 0.011615), hjust = 1, vjust = -1, size = 3) +
  annotate("text", x = max(bland_altman_data$Average), y = -0.011615, label = sprintf("Lower Limit: %.5f", -0.011615), hjust = 1, vjust = 1.5, size = 3) +
  labs(title = "Difference in Gratio (Manual vs. model_test)",
       x = "Gratio",
       y = "Difference in Gratio") +
  theme_minimal() +
  theme(
...
joined_data_man_default_all_final$name <- factor(joined_data_man_default_all_final$name, levels = c("manual", "default"))

default_model_manual_graph <- ggplot(joined_data_man_default_all_final, aes(x = name, y = gratio, col = name)) +
  geom_point(shape = 16, size = 5, alpha = 0.6, position = position_jitter(width = .1)) +
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "default" = "ADS test model")) +
...
ggplot(joined_y_final_final, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
   geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_all, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
  ...
  labs(
    x = "G-ratio of the Ground Truth", # new x label
    y = "G-ratio of the Default Model" # new y label
  ) +
  theme_classic() +
  theme(
    text = element_text(family = "Arial", size = 20),
...
# Let's remove all values where the axon diameter is <0.2um, as this is almost certain to be artifacts
joined_data_man_model14_61073_selection = subset(joined_data_man_model14_61073, axon_diam..um. >= 0.2, myelin_thickness..um. > 0.01)
...
# Let's do it for the manual and test datasets separately
morph_man_61073_selection = subset(morph_man_61073, axon_diam..um. >= 0.2, myelin_thickness..um. > 0.01)
morph_model15_61073_selection = subset(morph_model15_61073, axon_diam..um. >= 0.2, myelin_thickness..um. > 0.01)
...
model12_manual_graph <- ggplot(joined_data_man_model12_61073_final, aes(x = name, y = gratio, col = name)) +
  ...
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "12" = "model_12")) +
  scale_y_continuous(breaks = seq(0.45, 0.95, 0.05), limits = c(0.45, 0.95), expand = c(0, 0)) +
  scale_color_manual(values = c("manual" = "#00BFC4", "12" = "#F8766D")) +
  labs(
    title = "G-ratio comparison of ground truth vs model_12",
    x = "Measurement Method",
    y = "G-ratio"
  ) +
  theme_classic() +
...
# Add p-value annotation
model12_manual_graph <- model12_manual_graph + annotate("text", x = 1.5, y = 0.925, label = sprintf("p = %.4f", t_test_result_61073$p.value), size = 5, color = "black")
...
ggplot(model_compare_man_model12_61073, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_61073, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
...
model13_manual_graph <- ggplot(joined_data_man_model13_61073_final, aes(x = name, y = gratio, col = name)) +
  ...
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "13" = "model_13")) +
  scale_y_continuous(breaks = seq(0.45, 0.95, 0.05), limits = c(0.45, 0.95), expand = c(0, 0)) +
  scale_color_manual(values = c("manual" = "#00BFC4", "13" = "#F8766D")) +
  labs(
    title = "G-ratio comparison of ground truth vs model_13",
    x = "Measurement Method",
    y = "G-ratio"
  ) +
  theme_classic() +
...
# Add p-value annotation
model13_manual_graph <- model13_manual_graph + annotate("text", x = 1.5, y = 0.925, label = sprintf("p = %.4f", t_test_result_61073$p.value), size = 5, color = "black")
...
ggplot(model_compare_man_model13_61073, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_61073, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
...
model14_manual_graph <- ggplot(joined_data_man_model14_61073_final, aes(x = name, y = gratio, col = name)) +
  ...
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "model14" = "ADS model14 model")) +
  scale_y_continuous(breaks = seq(0.45, 0.95, 0.05), limits = c(0.45, 0.95), expand = c(0, 0)) +
...
# Add p-value annotation
model14_manual_graph <- model14_manual_graph + annotate("text", x = 1.5, y = 0.925, label = sprintf("p = %.4f", t_test_result_61073$p.value), size = 5, color = "black")
...
model15_manual_graph <- ggplot(joined_data_man_model15_61073_final, aes(x = name, y = gratio, col = name)) +
  ...
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .3, col = "black", size = .5) +
  stat_summary(fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.1, size = .4, col = "black") +
  scale_x_discrete(labels = c("manual" = "Ground Truth", "15" = "model_15")) +
  scale_y_continuous(breaks = seq(0.45, 0.95, 0.05), limits = c(0.45, 0.95), expand = c(0, 0)) +
  scale_color_manual(values = c("manual" = "#00BFC4", "15" = "#F8766D")) +
  labs(
    title = "G-ratio comparison of Ground truth vs model_15",
    x = "Measurement Method",
    y = "G-ratio"
  ) +
  theme_classic() +
...
# Add p-value annotation
model15_manual_graph <- model15_manual_graph + annotate("text", x = 1.5, y = 0.925, label = sprintf("p = %.4f", t_test_result_61073$p.value), size = 5, color = "black")
...
ggplot(model_compare_man_model15_61073, aes(x = gratio.x, y = gratio.y)) +
  geom_point(shape = 16, size = 5, alpha = .6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "red", size = 1) +  # Add the y=x line
  stat_smooth(method = "lm", fullrange = TRUE, fill = NA, size = 0.4) +
  stat_poly_eq(formula = my.formula_61073, aes(label = paste(after_stat(eq.label))), parse = TRUE, size = 6) + # adjust label.size for equation size
